...
public class MySQLTLSTest {
  ...
  public void setup() {
    vertx = Vertx.vertx();
    options = new DB2ConnectOptions(rule.options());
...
//  }
  ...
  public void testConnFailWithVerifyCaSslMode(TestContext ctx) {
    ...
    DB2Connection.connect(vertx, options, ctx.asyncAssertFailure(error -> {
  ...
  public void testPoolFailWithVerifyCaSslMode(TestContext ctx) {
    ...
    try {
      DB2Pool.pool(vertx, options, new PoolOptions());
  ...
  public void testChangeUser(TestContext ctx) {
    ...
    DB2Connection.connect(vertx, options, ctx.asyncAssertSuccess(conn -> {
      conn.query("SELECT current_user()", ctx.asyncAssertSuccess(res1 -> {
        ...
        DB2AuthOptions changeUserOptions = new DB2AuthOptions()
          .setUser("superuser")
