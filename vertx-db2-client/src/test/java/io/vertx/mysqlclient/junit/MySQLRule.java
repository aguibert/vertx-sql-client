...
public class MySQLRule extends ExternalResource {
  ...
  public synchronized DB2ConnectOptions startServer(DatabaseType databaseType, String databaseVersion, boolean ssl) throws Exception {
  ...
  protected void before() throws Throwable {
    ...
    if (isTestingWithExternalDatabase() && !ssl) {
      options = DB2ConnectOptions.fromUri(connectionUri);
      return;
    }

    // use an external database for tls testing
    if (isTlsTestingWithExternalDatabase() && ssl) {
      options = DB2ConnectOptions.fromUri(tlsConnectionUri);
      return;
    }
